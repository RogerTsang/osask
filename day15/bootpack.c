...
void HariMain(void) {
    ...
    /* Init Task */
    init_tasks();
    load_tr(3 * 8); /* Init CPU to process task_a */
    init_mt();
...
void task_b_main(void) {
	...
	timer_init(timer_put, &fifo, TDATA_PUT);
	timer_settime(timer_put, 100);

    putstr8_asc_lyr(lyr_back, 80, 160, COLOUR_WHITE, COLOUR_DCYAN, "Thread2", 7);
	...
	while(1) {
		...
		if (fifo32_status(&fifo) == 0) {
			...
			if (i == TDATA_PUT) {
                ...
                putstr8_asc_lyr(lyr_back, 160, 160, COLOUR_WHITE, COLOUR_DCYAN, string, 10);
	            timer_settime(timer_put, 100);
			}
		}
	}
